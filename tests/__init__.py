...
class TestValidFiles(object):
    ...
    def test_cd3_files(self):
        for cd3_fp in self._files_by_ext('.cd3'):
            yield self.check_cd3_file, cd3_fp

    def check_cd3_file(self, cd3_fp):
    ...
    def test_am_files(self):
        for am_fp in self._files_by_ext('.am'):
            yield self.check_am_file, am_fp

    def check_am_file(self, am_fp):
    ...
    def test_am_foreach_cd3(self):
        for cd3_fp in self._files_by_ext('.cd3'):
            yield self.check_am_for_cd3, cd3_fp

    def check_am_for_cd3(self, cd3_fp):
        cd3_fn = os.path.basename(cd3_fp)
